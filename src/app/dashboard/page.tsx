...
import { Loader2, Zap, Briefcase, MapPin, Check, RefreshCcw, ChevronLeft, ChevronRight, X, ExternalLink, CalendarDays, Award } from 'lucide-react';
        ...
        const unsub = onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                if (docSnap.exists()) {
                    ...
                    setProfile(data);
                    if (data.role && data.preferences?.location) {
                        fetchJobs(data);
                    } else {
                        setLoading(false);
                        setError("Please configure your Target Role and Location in the Profile section first.");
                    }
                }
            }
        });
    ...
    const fetchJobs = async (userData: any) => {
        try {
            ...
            const res = await fetch('/api/search-jobs', {
                ...
                body: JSON.stringify({
                    preferences: { title: userData.role, location: userData.preferences.location, minSalary: userData.preferences.minSalary },
                    resumeData: userData
                })
            });
        ...
        <div className="max-w-6xl mx-auto">
            {error && (
                <div className="bg-white/5 border border-white/10 p-8 rounded-3xl text-center glass mb-8">
                    ...
                    <a href="/dashboard/profile" className="mt-4 inline-block px-6 py-3 bg-primary text-white font-bold rounded-xl">Go to Profile</a>
                </div>
            )}
                ...
                <>
                    <div className="flex flex-col md:flex-row justify-between items-center bg-black/40 p-6 rounded-3xl border border-white/5 mb-8 backdrop-blur-md shadow-2xl glass">
                        <div>
                            ...
                            <p className="text-sm text-primary/80 font-medium">We found {jobs.length} highly relevant roles for your profile.</p>
                        </div>
                        <div className="flex flex-col sm:flex-row items-center gap-4 mt-4 md:mt-0">
                            <button
                                onClick={() => fetchJobs(profile)}
                                disabled={loading}
                                className="px-6 py-4 rounded-2xl font-bold bg-white/10 hover:bg-white/20 text-white transition-all flex items-center gap-2 hover:scale-105 active:scale-95 disabled:opacity-50 shadow-xl"
                            >
                                <RefreshCcw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} /> Refresh Matches
                            </button>
                        </div>
                    </div>
